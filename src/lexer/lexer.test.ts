    ...
    for (let i = 0; i < tests.length; i++) {
        ...
        it(`test ${i}`, () => {
            if (tok.Type !== tt.expectedType) {
                throw new Error(`tests[${i}] - tokentype wrong. expected=${tt.expectedType}, got=${tok.Type}`);
            }

            if (tok.Literal !== tt.expectedLiteral) {
                throw new Error(`tests[${i}] - literal wrong. expected=${tt.expectedLiteral}, got=${tok.Literal}`);
            }
        });
    }
})